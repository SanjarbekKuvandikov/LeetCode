import java.util.*;

public class Sum3 {
    public List<List<Integer>> threeSum(int[] nums) {
        List<List<Integer>> result = new ArrayList<>();
        Arrays.sort(nums);  // Sort the array to use two-pointer approach

        for (int i = 0; i < nums.length - 2; i++) {
            // Skip the same element to avoid duplicates
            if (i > 0 && nums[i] == nums[i - 1]) {
                continue;
            }

            int left = i + 1;  // Pointer starts from next element after i
            int right = nums.length - 1;  // Pointer starts from the end of the array
            int target = -nums[i];  // We are looking for two numbers that sum up to -nums[i]

            while (left < right) {
                int sum = nums[left] + nums[right];

                if (sum == target) {
                    // We found a triplet
                    result.add(Arrays.asList(nums[i], nums[left], nums[right]));

                    // Move pointers to skip duplicate elements
                    while (left < right && nums[left] == nums[left + 1]) left++;
                    while (left < right && nums[right] == nums[right - 1]) right--;

                    left++;
                    right--;
                } else if (sum < target) {
                    left++;  // Increase the left pointer to get a larger sum
                } else {
                    right--;  // Decrease the right pointer to get a smaller sum
                }
            }
        }

        return result;
    }
}
